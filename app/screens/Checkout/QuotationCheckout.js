...
const QuotationCheckout = ({ route, navigation }) => {
    ...
    try {
      if (
        ...
        const response = await fetch(
          `${BASE_URL}/hire/accept_qutation/${item?._id}/${userData.findUser._id}`,
          {
            method: "POST",
            headers: {
        ...
        if (response.ok) {
          console.log("Q Sent successful:", data.message);
          navigation.navigate("QuotationThankyou", { item });
        } else {
  ...
  return (
    <BackgroundImage source={require("../../assets/Home/bg_img.png")}>
      <View style={styles.container}>
        <Text style={styles.checkout_title}>Quotation Checkout </Text>

        <View style={styles.checkout_price_box}>
          <Text style={styles.total_price_title}>Total Price</Text>
          <Text style={styles.total_price}>$ {item?.amount}.00</Text>
        </View>
