...
import { useUserByCategoryQuery } from "../../Redux/Reducers/UserCategoriesReducer";
...
const UserListingCards = ({ navigation, route }) => {
  const { item } = route.params;

  const userByCategory = useUserByCategoryQuery(item._id, {
    skip: !item?._id,
  });
    ...
    <>
      <View style={styles.categories_heading}>
        <View style={styles.Category_container_Main}>
          <Text style={styles.category_Heading_Text}>
      ...
      <View style={styles.Main_container}>
        {userByCategory?.data?.userData?.length === 0 ? (
          <Text>Not Categories User Yet!</Text>
        ) : (
          <FlatList
            data={userByCategory?.data?.userData.filter(
